...
import { Edit, Trash2, MoreHorizontal, AlertTriangle, CheckCircle, Clock, Thermometer } from 'lucide-react'
...
export function IngredientList({ 
  ...
  useEffect(() => {
    fetchIngredients()
    fetchLocations()
  }, [currentPage, searchQuery, categoryFilter, locationFilter, stockFilter, refreshTrigger])
  ...
  const isAllSelected = ingredients.length > 0 && ingredients.every(i => selectedItems.includes(i.id))
  const isIndeterminate = selectedItems.length > 0 && !isAllSelected

  const getStockStatus = (currentStock: number, reorderPoint: number, expirationDate?: string) => {
            ...
            <TableBody>
              {loading ? (
                ...
                ingredients.map((ingredient) => {
                  const stockStatus = getStockStatus(ingredient.current_stock, ingredient.reorder_point, ingredient.expiration_date)
                  return (
                    <TableRow key={ingredient.id} className={selectedItems.includes(ingredient.id) ? "bg-muted/50" : ""}>
                      {onSelectionChange && (
                        <TableCell>
                          <Checkbox
                            checked={selectedItems.includes(ingredient.id)}
                            onCheckedChange={(checked) => handleSelectItem(ingredient.id, checked as boolean)}
                      ...
                      <TableCell>
                        <div>
                          ...
                          <div className="text-xs text-muted-foreground">
                            Reorder at {ingredient.reorder_point} {ingredient.unit}
                          </div>
                        </div>
                      </TableCell>
                      <TableCell>{formatCurrency(ingredient.unit_cost)}</TableCell>
                      <TableCell>{formatCurrency(ingredient.current_stock * ingredient.unit_cost)}</TableCell>
                      <TableCell>
                        <Badge 
                      ...
                      <TableCell>{getLocationName(ingredient.location_id)}</TableCell>
                      <TableCell>
                        <div>
                          {ingredient.supplier_name && (
                            <div className="text-sm">{ingredient.supplier_name}</div>
                          )}
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DialogTrigger asChild>
                                <DropdownMenuItem onSelect={() => setEditingIngredientId(ingredient.id)}>
                                ...
                                <AlertDialogContent>
                                  <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                      This will permanently delete the ingredient "{ingredient.name}".
