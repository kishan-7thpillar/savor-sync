    ...
    try {
      ...
      if (locationsData) {
        ...
        const locationsWithMetrics = locationsData.map((location) => ({
          ...location,
          status:
            Math.random() > 0.2 ? ("online" as const) : ("offline" as const),
        }));
  ...
  const fetchSalesData = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/sales?timeRange=${timeRange}&limit=100&weekStart=${weekStart.toISOString()}`
      );
      ...
      if (result.success) {
        ...
        setMetrics({
          ...
          activeLocations: locations.filter((l) => l.status === "online")
            .length,
        ...
        if (locations.length > 0) {
          const updatedLocations = locations.map((location, index) => {
            ...
            return {
              ...location,
              todaySales:
                result.data.metrics.totalSales *
                locationShare *
                (0.8 + Math.random() * 0.4),
              todayOrders: Math.round(
                result.data.metrics.totalOrders *
                  locationShare *
                  (0.8 + Math.random() * 0.4)
              ),
              avgOrderValue:
                result.data.metrics.avgOrderValue * (0.9 + Math.random() * 0.2),
            };
          });
              ...
              {locations.map((location) => (
                ...
                >
                  ...
                  <div className="flex items-center space-x-6 text-sm">
                    ...
                    <Badge
                      variant={
                        location.status === "online" ? "default" : "secondary"
                      }
                    >
                      {location.status || "offline"}
                    </Badge>
            ...
            {(topItems.length > 0
              ...
              .map((item, index) => (
                ...
                >
                  ...
                  <div className="text-right">
                    <div className="font-semibold">
                      {formatCurrency(item.estimatedSales)}
                    </div>
                    <div className="text-sm text-muted-foreground">
                      Est. revenue
                    </div>
